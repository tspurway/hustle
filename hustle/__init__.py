...
# Note that if the column function will change the data type of the original
# column, remember to create a new column and change its corresponding type.
# This matters when you need to store the query result back to the database.
...
def ip_aton(column):
    "column function for converting an integer IPv4 to a string"
    import mdb
    if column.type_indicator != mdb.MDB_STR:
        raise TypeError("Specified column should be of the string type")
    new_column = Column(column.name, column.table, column.index_indicator,
                        column.partition, mdb.MDB_UINT_32, compression_indicator=0,
                        rtrie_indicator=column.rtrie_indicator, alias=column.alias,
                        boolean = False, column_fn=_ip_aton)
    return new_column
